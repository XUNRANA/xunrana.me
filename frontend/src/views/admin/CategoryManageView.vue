<template>
  <div class="category-manage page-enter">
      ...
      <table class="ink-table">
        ...
        <tbody>
          <tr v-for="cat in categories" :key="cat.id">
            <td><span class="name-cell">{{ cat.name }}</span></td>
            <td><span class="mono-label">{{ cat.slug }}</span></td>
            <td><span class="desc-cell">{{ cat.description || '—' }}</span></td>
            <td class="col-count mono-label">{{ cat.articleCount }}</td>
            <td class="col-actions">
              <button class="action-btn" @click="openDialog(cat)">Edit</button>
              <button class="action-btn danger" @click="handleDelete(cat.id)">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
      <div v-if="categories.length === 0 && !loading" class="empty-state">
        <span class="mono-label">No categories yet</span>
      </div>
    </div>
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="0">
        ...
        <div class="form-field">
          <label class="field-label mono-label">Description</label>
...
const form = ref<CategoryDTO>({ name: '', slug: '', description: '' })
const rules = {
